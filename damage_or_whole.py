...
CHECKPOINT_NAME = './dow_checkpoint'
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    # Try out with an intermediate FC-Layer.
    layer_name = 'intermediate_layer'
    intermediate_layer_size = 256
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            initial_value = tf.truncated_normal(
                [bottleneck_tensor_size, intermediate_layer_size], stddev=0.001)
            layer_weights = tf.Variable(initial_value, name='final_weights')
            variable_summaries(layer_weights)

        with tf.name_scope('biases'):
            layer_biases = tf.Variable(tf.zeros([intermediate_layer_size]), name='final_biases')
            variable_summaries(layer_biases)

        with tf.name_scope('Wx_plus_b'):
            activation = tf.nn.relu(tf.matmul(bottleneck_input, layer_weights) + layer_biases)
            tf.summary.histogram('intermediate_activation', activation)
    ...
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            initial_value = tf.truncated_normal(
                [intermediate_layer_size, class_count], stddev=0.001)
            layer_weights = tf.Variable(initial_value, name='final_weights')
            variable_summaries(layer_weights)
        ...
        with tf.name_scope('Wx_plus_b'):
            logits = tf.matmul(activation, layer_weights) + layer_biases
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
    ...
    with tf.name_scope("confusion_matrix"):
        conf_matrix = tf.confusion_matrix(ground_truth_tensor, prediction, num_classes=3)

    tf.summary.scalar('accuracy', evaluation_step)
    return evaluation_step, prediction, conf_matrix
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
    ...
    (eval_session, _, bottleneck_input, ground_truth_input, evaluation_step,
     prediction, conf_matrix) = build_eval_session(module_spec, class_count)
    test_accuracy, predictions, conf_mat = eval_session.run(
        [evaluation_step, prediction, conf_matrix],
        feed_dict={
    ...
    tf.logging.info("Confusion Matrix : %s" % str(conf_mat))
...
def build_eval_session(module_spec, class_count):
    ...
    with eval_graph.as_default():
        ...
        evaluation_step, prediction, conf_matrix = add_evaluation_step(final_tensor,
                                                                       ground_truth_input)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--image_dir',
        type=str,
        default='./damage_or_whole',
        help='Path to folders of labeled images.'
    )
    parser.add_argument(
        '--output_graph',
        type=str,
        default='./dow_output_graph.pb',
        help='Where to save the trained graph.'
    )
    parser.add_argument(
        '--intermediate_output_graphs_dir',
        type=str,
        default='./dow_intermediate_graph/',
    ...
    parser.add_argument(
        '--saved_model_dir',
        type=str,
        default='./models/damage_or_whole/$(date +%s)/',
